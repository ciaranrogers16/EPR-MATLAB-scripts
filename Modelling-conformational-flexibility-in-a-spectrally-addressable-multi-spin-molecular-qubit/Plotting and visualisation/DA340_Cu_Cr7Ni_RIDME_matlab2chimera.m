...
PDBdata = pdb2mat('DA340_Chimera_130degree_final.pdb'); % read in data from PDB file 
%% Atomic coordinates in Gaussian frame [x,y,z], column vectors
% Geometry Optimised Coordinates
% A: Detection spin (Cu(II) Porphyrin)
Na=[PDBdata.X(354)    PDBdata.Y(354)   PDBdata.Z(354)];
Nb=[PDBdata.X(355)    PDBdata.Y(355)   PDBdata.Z(355)];
Nc=[PDBdata.X(356)    PDBdata.Y(356)   PDBdata.Z(356)];
Nd=[PDBdata.X(357)    PDBdata.Y(357)   PDBdata.Z(357)];
Cu=[PDBdata.X(425)    PDBdata.Y(425)   PDBdata.Z(425)];
A=cat(1,Na,Nb,Nc,Nd,Cu);
...
for mm=1:numel(p_values)
    ...
    GZ_Jmatrix = cat(2,gz_centre,J_matrix(mm,1)./6);
