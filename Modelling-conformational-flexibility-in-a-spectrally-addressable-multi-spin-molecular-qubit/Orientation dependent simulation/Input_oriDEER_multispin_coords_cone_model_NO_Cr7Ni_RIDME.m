...
opt.Pump_mw=29.95;       % Pumping pulse frequency in GHz (in RIDME - doesn't matter as BW of pump pulse infinite)
opt.B_obs=1215.8;        % field of the experiment in mT + b_corr
...
t=(0:0.004:1)';                 % DEER time trace in microseconds (us)it is good to match the time step to your experimental data if you want to fit one to the other
...
%% ---------------------------RUN THE CALCULATION---------------------------
for J = 1:length(p_pumpCr7Ni_detNO)
    p.N1 = p_pumpCr7Ni_detNO(:,J)';
    if opt.calcualte_DEER == 1
        ...
        save(['./Simulations_NO_Cr7Ni_RIDME_cone_model/Sim_J_',num2str(J),'.mat'],...
            'y_FTdeer_bin','f_sum_bin','v_b_bin','t','p_pumpCr7Ni_detNO');
    end
end