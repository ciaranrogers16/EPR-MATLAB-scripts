...
%
%
%  Try to avoid any plotting functions or graphic renderers as this will
%  make the save file very large
%==========================================================================
%%  DA340 Cu(II)-NO
%   Copper detection params. from DFT EPR-II calc.
%   NO params. defined geometrically from single crystal XRD structure
%
...
load('Cu_NO_DEER_DA340_Pvector_cone_model.mat')
...
opt.Det_mw=9.65;           % Detection pulse frequency in GHz
opt.Pump_mw=9.1;          % Pumping pulse frequency in GHz (in RIDME - doesn't matter as BW of pump pulse infinite)
opt.B_obs=322.0;             % field of the experiment in mT

% Detection pulse PARAMETERS
opt.Det_t180=52;          % time for pi detection pulses (ns)- edit to match experiment
...
opt.Pump_t180=20;         % time for pi Pump pulse (ns) - edit to match you experiment - in RIDME 0.00001 ns effectively infinite BW
...
N_A = [22,19,94];
opt.Pump_Ham.N1=struct('S',1/2,'g',[2.006 2.010 2.002],'gFrame',[0 0 0],...
    'Nucs','14N','A',N_A,'lwpp',[1.2]);
...
t=(0:0.008:1.25)';                % DEER time trace in microseconds (us)it is good to match the time step to your experimental data if you want to fit one to the other
opt.b0=(150:0.05:450)';   % field vector must cover CW EPR field range of both spin systems - make sure your spin system si resonant in this window.
...
opt.window = 0;                 % apply a window function before FFT
opt.gauss_wid = 0;              % use a gaussian window
opt.hann_wid = 0;               % use a hanning window
opt.hamm_wid =0;                % use a hamming window
opt.exp_wid =0;                 % use an exponential decay window
opt.zero_fill = 0;              % use zero-filling for FFT - you can perform hamming without zerofil but not zero fill without hamming
opt.zero_fill_number = 8192;    % number of points to zero fill to.

% logical operators for plotting and printing
opt.graph_CWEPR=1;                    % for graphing continuous wave EPR spectrum - graphs the CW spectrum of detection center and pump center(s) with the profiles of the detection and pump pulses - turn off if you are looping over multiple inputs as it requires manual input to continu calculation
...
opt.Print_coord=0;                    % print coordinates of system to the screen - these are not currently saved by the program as they depend on the geometry of the system.
...
for J = 1:10000
    ...
    if opt.calcualte_DEER == 1
        ...
        %  frequencies
        simfile = ['./Simulations_Pvals_cell_Cu_NO_DEER_100MHz/Sim_J_',num2str(J),'.mat'];
        eval(['save ',simfile])
    end
end