...
% Same p-vectors as 50K Cu-NO DEER (NO detection sequence)
...
opt.Pump_mw=34.6;       % Pumping pulse frequency in GHz (in RIDME - doesn't matter as BW of pump pulse infinite)
opt.B_obs=1209.5;        % field of the experiment in mT + b_corr
...
opt.Det_coordinates = NO_pump_overlay'; % coords of the detection center
opt.Det_Ham=struct('S',1/2,'g',[2.006 2.010 2.002],'gFrame',[0 0 0],...
    'Nucs','14N','A',[21.5556,23.7915,100.0972],'lw',[1.6183]);
...
Cu_A = [-60.4642 -60.4642 -423.9036]; % MHz
opt.Pump_Ham.N1=struct('S',1/2,'g',[2.0425 2.1604],'gFrame',[0 0 0],...
...
opt.Pump_coordinates.N1=cu_det; % coords of the first pump center
...
t=(0:0.004:1.2)';                 % DEER time trace in microseconds (us)it is good to match the time step to your experimental data if you want to fit one to the other
...
for J = 1:5000
    ...
    if opt.calcualte_DEER == 1
        ...
        save(['./Simulations_NO_Cu_RIDME_cone_model_6K/Sim_J_',num2str(J),'.mat'],...
            'y_FTdeer_bin','f_sum_bin','v_b_bin','t','p_pumpCu_detNO');
    end
end